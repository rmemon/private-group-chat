...
exports.listen = function(server){
  ...
  io.sockets.on('connection', function(socket){
    ...
    handleChoosingCreateGroup(socket);
    handleGroupMessaging(socket);
  });
}

function initializeConnection(socket){
  socket.emit('socket connect', 'Now you connect to socket');
...
function handleChoosingNicknames(socket){ 
  socket.on('choose nickname', function(nick, cb) {
    ...
    clients[ind].emit('current user', {id: ind, nick: nick});
...
function handlePrivateMessaging(socket){
  socket.on('private message', function(data){
    ...
    clients[data.userToPM].emit('private message', {from: from, msg: data.msg, user:data.msgFrom});
  });
}
...
function handleChoosingCreateGroup(socket){
  socket.on('create group', function(group, cb) {
	gId = groupNamesUsed.length;
    groupNamesUsed.push({id: gId, groupname:group.groupName,members:group.users});
    userInGroup[gId] = group.users;

    cb(null);

    group.users.forEach(function(user){    	
    	clients[user].emit('new group', {id: groupNamesUsed.length-1, group: group.groupName});
    });
    //io.sockets.emit('new group', {id: groupNamesUsed.length-1, group: group.groupName});
  });
}

function handleGroupMessaging(socket){
  socket.on('group message', function(data){
  	users = userInGroup[data.groupToChat];
	if(users.length > 0){
  		users.forEach(function(user){
    		clients[user].emit('group message', {groupId:data.groupToChat , msg:data.msg, from: data.from});
	 	 });
  	 }
  });
}
